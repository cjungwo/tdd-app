...
test.describe('guard', () => {
    test('if content not found, redirect to "/contents"', async ({ page, context }) => {
...
test.describe('main', () => {
    test('if visit, fetch ok', async ({
        ...
        await expect(helper.getMain.getByText(localizeDate(content.createAt))).toBeVisible();
...
test.describe('author-aside', () => {
    test('if visit, fetch ok', async ({
        ...
        await expect(helper.getAuthorAside.getByAltText(user.nickname)).toHaveAttribute('src', user.imgUrl);
